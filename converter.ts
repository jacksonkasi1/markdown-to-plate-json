 ...
 */
function enrichPlateJSON(
	plateNodes: Descendant[],
	// biome-ignore lint/suspicious/noExplicitAny: MDAST types are complex and dynamic
	mdNodes: any[],
		...
		if (isMatch) {
			...
			if (pNode.type === "li" && mNode.type === "listItem") {
				if (typeof mNode.checked === "boolean") {
					// biome-ignore lint/suspicious/noExplicitAny: Adding checked property dynamically
					(pNode as any).checked = mNode.checked;
				}
			} else if (pNode.type === "table" && mNode.type === "table") {
				if (mNode.align) {
					// biome-ignore lint/suspicious/noExplicitAny: Adding align property dynamically
...
function checkMatch(
	pNode: Element,
	// biome-ignore lint/suspicious/noExplicitAny: MDAST node types are dynamic
	mNode: any,
	definitions: Map<string, string>,
): boolean {
	...
	if (pNode.type === "ul" && mNode.type === "list" && !mNode.ordered)
		return true;
	if (pNode.type === "ol" && mNode.type === "list" && mNode.ordered)
		return true;
	...
	if (pNode.type === "h1" && mNode.type === "heading" && mNode.depth === 1)
		return true;
	if (pNode.type === "h2" && mNode.type === "heading" && mNode.depth === 2)
		return true;
	if (pNode.type === "h3" && mNode.type === "heading" && mNode.depth === 3)
		return true;
	if (pNode.type === "h4" && mNode.type === "heading" && mNode.depth === 4)
		return true;
	if (pNode.type === "h5" && mNode.type === "heading" && mNode.depth === 5)
		return true;
	if (pNode.type === "h6" && mNode.type === "heading" && mNode.depth === 6)
		return true;
	...
	if (
		pNode.type === "img" &&
		(mNode.type === "image" || mNode.type === "imageReference")
	)
		return true;

	if (
		pNode.type === "a" &&
		(mNode.type === "link" || mNode.type === "linkReference")
	) {
		// Check URL match to be sure
		// biome-ignore lint/suspicious/noExplicitAny: Accessing url property dynamically
...
function restoreLinkReference(
	...
	const children = mNode.children
		? // biome-ignore lint/suspicious/noExplicitAny: MDAST child node types are dynamic
			mNode.children.map((c: any) => {
